...
public partial class admin_TheoryCats : System.Web.UI.Page
{
    ...
    protected void loadCats()
    {
        ...
        string SQLstr = "select catId, catTitle, isPublished, psevdoName from theoryCategories order by catTitle";
        conn.Open();
        ...
        while (r.Read())
        {
            ...
            if (r["catId"].ToString() != "1")
            {
                ...
                string askText = "Удалить раздел теории \"" + r["catTitle"].ToString() + "\"?";
    ...
    protected void ibEdit_Click(object sender, ImageClickEventArgs e)
    {
        ...
        Response.Redirect("editTheoryCat.aspx?catId=" + ib.ID.Remove(0, 6));
    }
    ...
    protected void ibDelete_Click(object sender, ImageClickEventArgs e)
    {
        ...
        SqlConnection conn = new SqlConnection(connStr);
        string SQLstr = "proc_adminDeleteTheoryCat @catId, @userId, @eventTime";
        conn.Open();
        SqlCommand q = new SqlCommand(SQLstr, conn);
        ...
        conn.Close();
        Response.Redirect("theoryCats.aspx?status=deleted");
    }
