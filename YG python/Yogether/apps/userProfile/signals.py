    ...
    if not user_yg.social_data_loaded and user.socialaccount_set.filter(provider='vk'):
        ...
        try:
            ...
            if profile_pic_url and ('camera_400.png' not in profile_pic_url):
                ...
                user_yg.profile_pic_full.save(img_name, File(img_temp))
            ...
            print("Error loading vk data")

    if not user_yg.social_data_loaded and user.socialaccount_set.filter(provider='yandex'):

        try:
            user_dataset = user.socialaccount_set.filter(provider='yandex')[0]
            extra_data = user_dataset.extra_data
            ...
            print("Error loading yandex data")
